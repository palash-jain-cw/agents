...
class Agent:
    ...
    def format_request(self, request: BaseModel) -> List[dict]:
        return json.dumps(request.model_dump(), indent=2)

    def run(self, request: BaseModel) -> Tuple[BaseModel, dict]:
        request_json = self.format_request(request)
        response, usage = self.client.query(
            cached_content=[self.instructions],
            variable_content=[request_json],
